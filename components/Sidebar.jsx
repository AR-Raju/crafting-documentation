...
import { getDocumentsByAuthor, getDocumentsByTag } from "@/utils/doc-util";
...
const Sidebar = ({ docs }) => {
  ...
  const [nonRootNotesGrouped, setNonRootNodesGrouped] = useState({});
  ...
  useEffect(() => {
    ...
    if (pathName.includes("/tags")) {
      ...
      matchedDocs = getDocumentsByTag(docs, categories);
    }
            ...
            {rootNodes.map((rootNode) => {
              return (
                <li key={rootNode.id} className="relative">
                  ...
                  {nonRootNotesGrouped[rootNode.id] && (
                    <ul role="list" className="border-l border-transparent">
                      {nonRootNotesGrouped[rootNode.id].map((subRoot) => (
                        <li key={subRoot.id}>
                          <Link
                            aria-current="page"
